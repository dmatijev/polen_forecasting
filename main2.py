...
def train(model, train_loader, valid_loader, test_loader, loss_fn, optimizer, scheduler, device, target, bestInRuns):
    ...
    for i in range(epochs):
        ...
        if valid_epoch_loss/len(valid_loader) < bestLoss:
            if valid_epoch_loss/len(valid_loader) < bestInRuns:
                print('best model found, saving...')
                torch.save(model.state_dict(), f'proba_best.weights')
                bestInRuns = valid_epoch_loss/len(valid_loader)
...
if __name__ == "__main__":
    ...
    bInRuns = 1e9
    for it in range(1):
        model = Net2(input_dim,  hidden_dim = hidd_dim, hidden_dim2 = hidd_dim2, seq_len = seq_len, attention = att)
        ...
        bestLoss = train(model, train_loader, val_loader, test_loader, loss_fn, optimizer, scheduler, device=device, target=TARGET, bestInRuns=bInRuns)
        if bestLoss < bInRuns:
            bInRuns = bestLoss
        vektorVrijednosti.append(bestLoss)
        print(f"run {it}: best loss = {bestLoss}")
    print(vektorVrijednosti)
    print(sum(vektorVrijednosti)/len(vektorVrijednosti))